...
class MediaService(ComfyBaseService):
    ...
    Usage:
        ...
        media = await baru_pixelle.media(
            prompt="a cat",
            workflow="image_flux.json"
        )
        
        # List available workflows
        workflows = baru_pixelle.media.list_workflows()
    """
    ...
    def _scan_workflows(self):
        ...
        """
        from baru_pixelle.utils.os_util import list_resource_dirs, list_resource_files, get_resource_path
    ...
    async def __call__(
        ...
        # Output path (Gemini direct mode only — workflow modes save where ComfyKit saves)
        ...
        **params
        ...
        Examples:
            ...
            media = await baru_pixelle.media(prompt="a beautiful cat")
            if media.is_image:
