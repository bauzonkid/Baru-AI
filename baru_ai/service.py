...
from baru_ai.services.llm_service import LLMService
from baru_ai.services.tts_service import TTSService
from baru_ai.services.media import MediaService
from baru_ai.services.image_analysis import ImageAnalysisService
from baru_ai.services.video_analysis import VideoAnalysisService
from baru_ai.services.video import VideoService
from baru_ai.services.frame_processor import FrameProcessor
from baru_ai.services.persistence import PersistenceService
from baru_ai.services.history_manager import HistoryManager
...
class PixelleVideoCore:
    ...
    Usage:
        ...
        print(f"Using LLM: {baru_ai.llm.active}")
        print(f"Available TTS: {baru_ai.tts.available}")
    ...
    async def _get_or_create_comfykit(self) -> ComfyKit:
        ...
        if self._comfykit is None or self._comfykit_config_hash != current_hash:
            ...
            if self._comfykit is not None:
                logger.info("ðŸ”„ ComfyUI configuration changed, recreating ComfyKit instance...")
                try:
            ...
            logger.info("âœ¨ Creating ComfyKit instance...")
            ...
            logger.info("âœ… ComfyKit instance created")
    ...
    async def initialize(self):
        ...
        if self._initialized:
            logger.warning("Baru-AI already initialized")
            return
        
        logger.info("ðŸš€ Initializing Baru-AI...")
        ...
        logger.info("âœ… Baru-AI initialized successfully\n")
    ...
    async def cleanup(self):
        ...
        Example:
            await baru_ai.cleanup()
        """
        if self._comfykit:
            logger.info("ðŸ§¹ Closing ComfyKit session...")
            try:
                await self._comfykit.close()
                logger.info("âœ… ComfyKit session closed")
            except Exception as e:
    ...
    def project_name(self) -> str:
        ...
        return self.config.get("project_name", "Baru-AI")
