...
class VideoService:
    ...
    def merge_audio_video(
        ...
        if auto_adjust_duration:
            ...
            if diff < 0:
                # Video shorter than audio â†’ Must pad to avoid black screen
                logger.warning(f"âš ï¸ Video shorter than audio by {abs(diff):.2f}s, padding required")
                ...
                logger.info(f"ðŸ“Œ Padded video to {audio_duration:.2f}s")
            
            elif diff > duration_tolerance:
                # Video significantly longer than audio â†’ Trim
                logger.info(f"âš ï¸ Video longer than audio by {diff:.2f}s (tolerance: {duration_tolerance}s)")
                video = self._trim_video_to_duration(video, audio_duration)
                video_duration = audio_duration  # Update duration after trimming
                logger.info(f"âœ‚ï¸ Trimmed video to {audio_duration:.2f}s")
            
            else:  # 0 <= diff <= duration_tolerance
                # Video slightly longer but within tolerance â†’ Keep as-is
                logger.info(f"âœ… Duration acceptable: video={video_duration:.2f}s, audio={audio_duration:.2f}s (diff={diff:.2f}s)")
    ...
    def _get_unique_temp_path(self, prefix: str, original_filename: str) -> str:
        ...
        """
        from baru_ai.utils.os_util import get_temp_path
