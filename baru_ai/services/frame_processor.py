...
Orchestrates: TTS â†’ Image Generation â†’ Frame Composition â†’ Video Segment
...
from baru_ai.models.storyboard import Storyboard, StoryboardFrame, StoryboardConfig


class FrameProcessor:
    """Frame processor"""
    
    def __init__(self, baru_ai_core):
        """
        Initialize
        
        Args:
            baru_ai_core: PixelleVideoCore instance
        """
        self.core = baru_ai_core
    ...
    async def __call__(
        ...
        try:
            ...
            logger.info(f"âœ… Frame {frame.index} completed")
            return frame

        except Exception as e:
            logger.error(f"âŒ Failed to process frame {frame.index}: {e}")
            raise
    ...
    async def _step_generate_audio(
        ...
        from baru_ai.utils.os_util import get_task_frame_path
    ...
    async def _step_compose_frame(
        ...
        logger.debug(f"  âœ“ Frame composed: {composed_path}")
    ...
    async def _download_media(
        ...
        - ``http://`` / ``https://`` â†’ download with httpx.
        - Local path (Imagen / Gemini direct providers write to
          ``output/<uuid>.png`` straight out of the SDK) â†’ copy in
        ...
        # ``get_task_frame_path`` doesn't mkdir â€” the http branch
