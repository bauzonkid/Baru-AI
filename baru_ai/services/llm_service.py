...
# Cloudflare in front of sáº¿p's 9router origin (yohomin.com) blocks the
...
# tunnel. Same workaround Baru-YTB uses â€” see baru_ytb/_ai_provider.py.
_BENIGN_HEADERS = {
    "User-Agent": (
...
class LLMService:
    ...
    def _create_client(
        ...
        # Create client. Always inject the benign UA + extended timeout â€”
...
You MUST respond with ONLY a valid JSON object (no markdown, no extra text)."""
    ...
    def active(self) -> str:
        ...
        Example:
            print(f"Using model: {baru_ai.llm.active}")
        """
