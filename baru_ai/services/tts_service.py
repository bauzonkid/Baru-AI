...
from baru_ai.utils.tts_util import edge_tts
from baru_ai.tts_voices import speed_to_rate
...
class TTSService(ComfyBaseService):
    ...
    Usage:
        ...
        audio_path = await baru_ai.tts(text="Hello, world!")
        
        # Use specific workflow
        audio_path = await baru_ai.tts(
            text="ä½ å¥½ï¼Œä¸–ç•Œï¼",
            workflow="tts_edge.json"
        )
        
        # List available workflows
        workflows = baru_ai.tts.list_workflows()
    """
    ...
    async def _call_local_tts(
        ...
        final_voice = voice or local_config.get("voice", "en-US-AriaNeural")
        ...
        logger.info(f"ðŸŽ™ï¸  Using local Edge TTS: voice={final_voice}, speed={final_speed}x (rate={rate})")
        ...
        try:
            ...
            logger.info(f"âœ… Generated audio (local Edge TTS): {output_path}")
            return output_path
    ...
    async def _call_comfyui_workflow(
        ...
        **params
        ...
        """
        logger.info(f"ðŸŽ™ï¸  Using workflow: {workflow_info['key']}")
        ...
        try:
            ...
            if hasattr(result, 'audios') and result.audios:
                ...
                logger.debug(f"âœ… Found audio in result.audios: {audio_path}")
            # Check for files in result.files
            elif hasattr(result, 'files') and result.files:
                audio_path = result.files[0]
                logger.debug(f"âœ… Found audio in result.files: {audio_path}")
            # Check in outputs dictionary
                ...
                for key, value in result.outputs.items():
                    if isinstance(value, str) and any(value.endswith(ext) for ext in ['.mp3', '.wav', '.flac']):
                        audio_path = value
                        logger.debug(f"âœ… Found audio in result.outputs[{key}]: {audio_path}")
                        break
            
            if not audio_path:
                logger.error("No audio file generated")
                logger.error(f"âŒ Result analysis:")
            ...
            if output_path and audio_path.startswith(('http://', 'https://')):
                ...
                logger.info(f"âœ… Generated audio (ComfyUI): {output_path}")
                return output_path
            
            logger.info(f"âœ… Generated audio (ComfyUI): {audio_path}")
            return audio_path
