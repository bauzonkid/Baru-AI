...
async def generate_title(
    ...
    from baru_ai.prompts import build_title_generation_prompt
...
async def generate_narrations_from_topic(
    ...
    """
    from baru_ai.prompts import build_topic_narration_prompt
...
async def generate_narrations_from_content(
    ...
    Returns:
        List of narration texts
    """
    from baru_ai.prompts import build_content_narration_prompt
...
async def split_narration_script(
    ...
    Args:
        ...
        split_mode: Splitting strategy
            ...
            - "sentence": Split by sentence-ending punctuation (ã€‚.!?ï¼ï¼Ÿ)
    ...
    if split_mode == "paragraph":
        ...
        logger.info(f"âœ… Split script into {len(narrations)} segments (by paragraph)")
    
    elif split_mode == "line":
        # Split by single newline (original behavior)
        narrations = [line.strip() for line in script.split('\n') if line.strip()]
        logger.info(f"âœ… Split script into {len(narrations)} segments (by line)")
    
    elif split_mode == "sentence":
        # Split by sentence-ending punctuation
        # Supports Chinese (ã€‚ï¼ï¼Ÿ) and English (.!?)
        ...
        logger.info(f"âœ… Split script into {len(narrations)} segments (by sentence)")
...
async def generate_image_prompts(
    ...
    """
    from baru_ai.prompts import build_image_prompt_prompt
    ...
    for batch_idx, batch_narrations in enumerate(batches, 1):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                # Success!
                logger.info(f"âœ… Batch {batch_idx} completed successfully ({len(batch_prompts)} prompts)")
    ...
    logger.info(f"âœ… Generated {len(all_prompts)} image prompts")
    return all_prompts
...
async def generate_video_prompts(
    ...
    """
    from baru_ai.prompts.video_generation import build_video_prompt_prompt
    ...
    for batch_idx, batch_narrations in enumerate(batches, 1):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                all_prompts.extend(batch_prompts)
                logger.info(f"âœ“ Batch {batch_idx} completed: {len(batch_prompts)} video prompts")
                ...
                logger.warning(f"âœ— Batch {batch_idx} attempt {attempt} failed: {e}")
                if attempt >= max_retries:
                    raise
                logger.info(f"Retrying batch {batch_idx}...")
    
    logger.info(f"âœ… Generated {len(all_prompts)} video prompts")
    return all_prompts
